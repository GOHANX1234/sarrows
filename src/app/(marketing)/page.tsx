...
import { Play, Info, ChevronRight, Film, Tv, Globe, Subtitles, Mic, Sparkles, Send, Smartphone, Download } from "lucide-react";
...
const DEFAULT_APK_URL = "https://github.com/GohanDev3/apks/releases/download/M%26A_V3/M.A_3.0.0.apk";
const DEFAULT_VERSION_NAME = "v3.0.0";

async function getLatestAppRelease(): Promise<AppReleaseInfo> {
  try {
    await connectDB();
    // Check for active update for android/all
    let active = await AppVersion.findOne({
      isActive: true,
      platform: { $in: ["android", "all"] },
    })
      .sort({ versionCode: -1 })
      .select("versionName downloadUrl")
      .lean() as any;

    // Fallback if no active flag set, grab highest versionCode
    if (!active) {
      active = await AppVersion.findOne({
        platform: { $in: ["android", "all"] },
      })
        .sort({ versionCode: -1 })
        .select("versionName downloadUrl")
    ...
    if (active && active.downloadUrl) {
      let vName = active.versionName ? String(active.versionName).trim() : "3.0.0";
      if (!vName.toLowerCase().startsWith("v")) {
        vName = `v${vName}`;
      }
      return {
    ...
    console.error("Error fetching latest app version for marketing page:", err);
  }
...
export default async function LandingPage() {
  const [catalog, latestFeatured, appRelease] = await Promise.all([
    getCatalogPreview(),
    getLatestFeatured(),
    getLatestAppRelease(),
  ]);
  ...
  return (
    <div className="min-h-screen bg-sarrows-darker overflow-x-hidden">
          ...
          <div className="max-w-2xl">
            {featured && (
              <div className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-sarrows-red mb-4 sm:mb-5 uppercase tracking-[0.15em] motion-safe:animate-fade-in">
                ...
                Now Streaming &middot; {featured.type === "movie" ? "Movie" : "Anime"}
              </div>
            )}
            ...
            {/* Redesigned CTAs: clean, compact, balanced */}
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 max-w-xl">
              <Link href="/signup" className="btn-primary text-sm sm:text-base px-6 py-3 font-bold justify-center shadow-lg shadow-sarrows-red/25">
                <Play className="w-4 h-4 fill-current" />
                Start Watching Free
              </Link>
              <div className="flex items-center gap-2.5">
                <Link href="/movies" className="btn-secondary text-sm px-4 py-3 justify-center flex-1 sm:flex-initial">
                  <Info className="w-4 h-4" />
                  Browse Library
                </Link>
                <a
                  href={appRelease.downloadUrl}
                  className="inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-semibold text-xs sm:text-sm text-emerald-400 bg-emerald-950/40 border border-emerald-500/25 hover:bg-emerald-900/40 transition-all duration-200 active:scale-[0.98] flex-1 sm:flex-initial"
                  title="Download Android APK"
                >
                  <Download className="w-4 h-4 text-emerald-400" />
                  <span>Get App</span>
                  <span className="text-[10px] bg-emerald-500/20 text-emerald-300 px-1.5 py-0.5 rounded font-bold">{appRelease.versionName}</span>
                </a>
              </div>
            </div>
      ...
      <section className="py-10 sm:py-14 px-5 sm:px-6 md:px-10 max-w-6xl mx-auto">
        <ScrollReveal>
          <div
            className="rounded-3xl p-6 sm:p-8 md:p-10 relative overflow-hidden flex flex-col md:flex-row items-center gap-8 md:gap-12"
            style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.08)" }}
          >
            {/* Green glow */}
            <div
              className="absolute -top-16 -left-16 w-72 h-72 rounded-full pointer-events-none"
              style={{ background: "radial-gradient(ellipse, rgba(34,165,94,0.18), transparent 70%)", filter: "blur(50px)" }}
            />

            {/* App icon block */}
            <div className="relative flex-none">
              <div
                className="w-24 h-24 sm:w-28 sm:h-28 rounded-3xl flex items-center justify-center shadow-2xl"
                style={{ background: "linear-gradient(135deg, #1a7a4a 0%, #22a55e 100%)", boxShadow: "0 12px 40px rgba(34,165,94,0.4)" }}
              >
                <Smartphone className="w-12 h-12 sm:w-14 sm:h-14 text-white" />
              </div>
              {/* "NEW" badge */}
              <span
                className="absolute -top-2 -right-2 text-[10px] font-black uppercase tracking-wider text-white px-2 py-0.5 rounded-full"
            ...
            <div className="relative flex-1 text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-emerald-400 mb-3">
                <Smartphone className="w-3.5 h-3.5" /> Official Android App
              </div>
              <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-white mb-2">
                Watch on your phone.<br className="hidden sm:block" />
                Anytime, anywhere.
              </h2>
              <p className="text-gray-400 text-sm sm:text-base mb-6 max-w-lg mx-auto md:mx-0">
                The Sarrows Android app brings your full library to your pocket - movies, anime, dubs &amp; subs, all offline-ready. Version {appRelease.versionName.replace(/^v/i, '')} is out now.
              </p>
              <div className="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-3">
                <a
                  href={appRelease.downloadUrl}
                  className="inline-flex items-center gap-2.5 px-6 py-3 rounded-xl font-bold text-sm sm:text-base text-white transition-all duration-200 hover:opacity-90 hover:scale-[1.02] active:scale-[0.98]"
                  style={{ background: "linear-gradient(135deg, #1a7a4a 0%, #22a55e 100%)", boxShadow: "0 4px 24px rgba(34,165,94,0.35)" }}
                >
                  <Download className="w-4 h-4 sm:w-5 sm:h-5" />
                  Download APK - Free
                </a>
                <span className="text-gray-500 text-xs">{appRelease.versionName} &middot; Android</span>
              </div>
              <p className="text-gray-600 text-xs mt-3">
                Direct APK install - enable "Install from unknown sources" in your Android settings.
              </p>
            </div>
          </div>
        </ScrollReveal>
          ...
          >
            ...
            <div className="relative">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                <Link href="/signup" className="btn-primary text-base px-8 py-3 inline-flex">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-base text-white transition-all duration-200 hover:opacity-90 active:scale-[0.98]"
              ...
              <p className="text-gray-600 text-xs mt-5">
                ...
                <Link href="/terms" className="text-gray-400 hover:text-gray-200 underline underline-offset-2 transition">Terms of Service</Link>
                {" "}and{" "}
                <Link href="/privacy" className="text-gray-400 hover:text-gray-200 underline underline-offset-2 transition">Privacy Policy</Link>.
              </p>
            </div>
          </div>
        </ScrollReveal>
      ...
      <footer className="border-t border-white/[0.06] py-8 px-5 sm:px-6" style={{ paddingBottom: "calc(2rem + env(safe-area-inset-bottom))" }}>
        <div className="flex flex-col sm:flex-row items-center justify-between max-w-6xl mx-auto gap-4">
          ...
          <p className="text-gray-600 text-xs">&copy; {new Date().getFullYear()} Sarrows. All rights reserved.</p>
