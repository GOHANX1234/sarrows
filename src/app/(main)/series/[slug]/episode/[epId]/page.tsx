...
export default async function WebSeriesEpisodePage({ params }: Props) {
  const { slug, epId } = await params;

  // Reject malformed ObjectIds early — prevents a 500 from Mongoose
  if (!mongoose.isValidObjectId(epId)) notFound();

  const session = await auth();
  await connectDB();

  const [series, episode] = await Promise.all([
    Series.findOne({ slug, publishStatus: "published", type: "series" }).lean(),
    Episode.findById(epId).select("+videoUrl +videoType").lean(),
  ]);

  if (!series || !episode) notFound();

  // Verify the episode actually belongs to this series
