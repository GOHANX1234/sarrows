...
import { Search, User, LogOut, Settings, Bookmark, Home, Film, Tv, Monitor, ChevronDown } from "lucide-react";
...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { href: "/series", label: "Web Series", icon: Monitor },
  ];
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-4 md:px-8 h-14 sm:h-16 max-w-screen-2xl mx-auto">
          ...
          {/* Logo â€” always shows mark + wordmark */}
      ...
      >
        <div className="bg-black/85 backdrop-blur-xl border-t border-white/10 px-2 py-1 shadow-2xl">
          <div className="flex items-center justify-around">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`relative flex flex-col items-center gap-0.5 flex-1 py-1.5 rounded-xl transition-all duration-200 ${
                    active ? "text-sarrows-red font-semibold" : "text-gray-400 hover:text-gray-200"
                  }`}
                >
                  <link.icon className={`w-5 h-5 transition-transform duration-200 ${active ? "scale-110 text-sarrows-red" : ""}`} />
                  <span className={`text-[10px] tracking-tight transition-colors ${active ? "text-sarrows-red font-bold" : "text-gray-400"}`}>
                    {link.label}
                  </span>
                  {active && (
                    <span className="absolute -top-1 w-8 h-0.5 bg-sarrows-red rounded-full shadow-[0_0_8px_rgba(239,68,68,0.8)]" />
                  )}
                </Link>
              );
            })}
            ...
            <Link
              href="/search"
              className={`relative flex flex-col items-center gap-0.5 flex-1 py-1.5 rounded-xl transition-all duration-200 ${
                isActive("/search") ? "text-sarrows-red font-semibold" : "text-gray-400 hover:text-gray-200"
              }`}
            >
              <Search className={`w-5 h-5 transition-transform duration-200 ${isActive("/search") ? "scale-110 text-sarrows-red" : ""}`} />
              <span className={`text-[10px] tracking-tight ${isActive("/search") ? "text-sarrows-red font-bold" : "text-gray-400"}`}>
                Search
              </span>
              {isActive("/search") && (
                <span className="absolute -top-1 w-8 h-0.5 bg-sarrows-red rounded-full shadow-[0_0_8px_rgba(239,68,68,0.8)]" />
            ...
            {session?.user ? (
              <Link
                ...
                className={`relative flex flex-col items-center gap-0.5 flex-1 py-1.5 rounded-xl transition-all duration-200 ${
                  pathname.startsWith("/profile") ? "text-sarrows-red font-semibold" : "text-gray-400"
                }`}
              >
                <div className={`w-5 h-5 rounded-md bg-gradient-to-br from-sarrows-red to-red-700 flex items-center justify-center transition-transform ${pathname.startsWith("/profile") ? "scale-110 ring-2 ring-sarrows-red/50" : ""}`}>
                  <span className="text-white text-[9px] font-bold leading-none">
                    {session.user.name?.[0]?.toUpperCase()}
                  </span>
                </div>
                <span className={`text-[10px] tracking-tight ${pathname.startsWith("/profile") ? "text-sarrows-red font-bold" : "text-gray-400"}`}>Profile</span>
              ...
              <Link
                href="/login"
                className="flex flex-col items-center gap-0.5 flex-1 py-1.5 rounded-xl text-gray-400 hover:text-white transition-all duration-200"
              >
