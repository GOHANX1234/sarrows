...
export default function AnimeForm({ genres, initial, onSuccess, onGenreCreated }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    animeSaltUrl: initial?.animeSaltUrl || "",
  ...
  const fillFromJikan = async (result: any) => {
    ...
    try {
      ...
      // Fetch trailer from TMDB — search by title, take the first TV result, pull only trailerUrl.
      // AniList handles all other metadata; TMDB is used solely for the trailer here.
      if (result.title) {
        tasks.push(
          fetch(`/api/admin/tmdb/search?q=${encodeURIComponent(result.title)}&type=tv`)
            .then((res) => (res.ok ? res.json() : null))
            .then((searchData) => {
              const firstMatch = searchData?.results?.[0];
              if (!firstMatch?.externalId) return;
              return fetch(`/api/admin/tmdb/tv/${firstMatch.externalId}`)
                .then((res) => (res.ok ? res.json() : null))
                .then((details) => {
                  if (details?.trailerUrl) {
                    setForm((prev) => ({ ...prev, trailerUrl: details.trailerUrl }));
                  }
                });
            })
            .catch(() => {}) // Non-fatal: trailer stays blank if TMDB has no match
        );
      }
  ...
  return (
    <form onSubmit={submit} className="bg-sarrows-card border border-sarrows-border rounded-xl p-6 space-y-5 max-w-2xl">
      ...
      <div className="border border-sarrows-border rounded-lg p-4 bg-white/2">
        ...
        {loadingDetails && <p className="text-xs text-gray-500 mt-2">Fetching cast, genres and trailer…</p>}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          <label className="block text-sm font-medium text-gray-300 mb-1">AnimeSalt Series URL (optional)</label>
          <input
            type="url"
            value={form.animeSaltUrl}
            onChange={(e) => set("animeSaltUrl", e.target.value)}
            className="input-field"
            placeholder="https://animesalt.link/series/your-anime/"
          />
          <p className="text-[11px] text-gray-600 mt-1">
            Used to auto-fill AnimeSalt iframe URLs for episodes. If blank, the app tries the anime title as the slug.
          </p>
        </div>
        <div>
